...
Асортимент товару на сьогодні: {products_info}
...
"""
    # Создаем полноценную чат-сессию с системной инструкцией
...
async def handle_message(message: types.Message):
    ...
    # Если сессия почему-то пропала (перезапуск бота), создаем её заново
    if user_id not in user_chats:
        await cmd_start(message)

    try:
        # Отправляем сообщение в текущую сессию чата
